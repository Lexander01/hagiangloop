...
import { PRICE_PER_PAX_USD, DEPOSIT_FRACTION, MAX_SPOTS } from '../lib/constants'
...
export default function BookingForm({ selectedDate, availableSpots }: Props) {
  ...
  const total      = PRICE_PER_PAX_USD * pax
  const deposit    = Math.round(total * DEPOSIT_FRACTION)
  const cashDue    = total - deposit
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-stone-200 p-6 space-y-5">
      ...
      >
        {loading ? 'Redirecting to payment…' : `Pay $${deposit} Deposit →`}
      </button>

      <p className="text-xs text-center text-stone-400">
        Secure payment via Stripe. The remaining ${cashDue} is paid in cash to your guide on the day.
      </p>
    </form>
  )
}
